...
## Output files
* `output_files/model_3.20210928.rds` = RDS of fitted model
...
my_plot <- function(pdat.ancestry, i, mega_pdat.ancestry, reference_level, save_fig_bool, width, height, today, fig_dir) {
    ...
    if (save_fig_bool) {
      fn = sanitize_var_for_fn(file.path(fig_dir, sprintf("figure_1.tissue_ancestry_effects.%s_vs_%s.%s.pdf", i, reference_level, today)))
...
plot_sex <- function(t5, is_transformed) {
  tmp = t5$pdat.sex
  tmp = tmp[with(tmp, order(Estimate)),]
  tmp$tissue = factor(tmp$tissue, levels=tmp$tissue)
  ...
  if (save_fig_bool) {
    fn = sanitize_var_for_fn(file.path(fig_dir, sprintf("figure_1.tissue_sex_effects%s.%s.pdf", suffix, today)))
    pdf(fn, width=5.25, height=3)
  }
...
```{r}
# Save 
output_file = sprintf("output_files/model_3.%s.rds", today)
...
if (save_fig_bool) {
  fn = file.path(fig_dir, sprintf("figure_1.covariate_f_statistics.%s.pdf", today))
  pdf(fn, width=4, height=3)
}
...
my_plot(t5_white$pdat.ancestry, i="African American", mega_pdat.ancestry, "White", save_fig_bool=save_fig_bool, width=5, height=2, today, fig_dir)
```

```{r, fig.height=3.5, fig.width=5}
p2 = my_plot(t5_white$pdat.ancestry, i="Asian", mega_pdat.ancestry, "White", save_fig_bool=save_fig_bool, width=5, height=3.5, today, fig_dir)
```

```{r, fig.height=2.75, fig.width=5}
p2 = my_plot(t5_aa$pdat.ancestry, i="Asian", mega_pdat.ancestry, "African American", save_fig_bool=save_fig_bool, width=5, height=2.75, today, fig_dir)
```

#### Sex
```{r, fig.height=3, fig.width=5.25}
plot_sex(t5_white, is_transformed=FALSE)
```

#### Age
```{r, fig.height=4, fig.width=5}
tmp = t5_white$pdat.age
tmp = tmp[with(tmp, order(Estimate)),]
tmp$tissue = factor(tmp$tissue, levels=tmp$tissue)

if (save_fig_bool) {
  fn = sanitize_var_for_fn(file.path(fig_dir, sprintf("figure_1.tissue_age_effects.%s.pdf", today)))
...
# rpt was run in parallel on a cluster to decrease the amount of wall clock time.  See preprocess_steps/get_rpt_estimates.R.
rdat_fn = "preprocess_steps/repeat_estimates.20210928.Rdata"
